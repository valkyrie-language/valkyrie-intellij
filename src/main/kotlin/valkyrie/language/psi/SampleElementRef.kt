...
abstract class SampleElementRef(element: ValkyrieNamepathNode) :
    PsiReferenceBase<ValkyrieNamepathNode>(element, TextRange(0, element.text.length)) {
...
//		                   " at "+Integer.toHexString(myElement.hashCode())+")");
        val scope = myElement!!.context ?: return null
    ...
    override fun isReferenceTo(def: PsiElement): Boolean {
        ...
        if (def is ValkyrieNamepathNode && isDefSubtree(def.getParent())) {
            def = def.getParent()
        }
        if (isDefSubtree(def)) {
