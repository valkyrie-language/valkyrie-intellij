...
class ValkyrieUsageVisionProvider : DaemonBoundCodeVisionProvider {
    ...
    private fun computeForFile(file: PsiFile): List<Pair<TextRange, CodeVisionEntry>> {
        ...
        // 查找所有类声明
        val classDeclarations = PsiTreeUtil.findChildrenOfType(file, ValkyrieClassDeclaration::class.java)
        println("[ValkyrieCodeVision] Found ${classDeclarations.size} class declarations in ${file.name}")

        classDeclarations.forEach { classDecl ->
            val usageCount = countUsages(classDecl)
            println("[ValkyrieCodeVision] Class ${classDecl.name} has $usageCount usages")
            if (usageCount > 0) {
                val range = getDeclarationRange(classDecl)
                val entry = createUsageEntry(usageCount, "usage")
                result.add(range to entry)
                println("[ValkyrieCodeVision] Added code vision entry for ${classDecl.name}")
            }
        }
