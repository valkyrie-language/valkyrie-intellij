 ...
 * Widget 语句实现
 */
class ValkyrieWidgetDeclaration(node: ASTNode) : ValkyrieElementNode(node),
    PsiNameIdentifierOwner,
    HasAnnotation,       // ↯attribute widget X { }
    HasTypeParameter,    // widget X<T> { }
    HasInheritParameter, // widget X(A) { }
    HasObjectBody,       // widget X { object_body }
    HasHighlighter
{
    ...
    override val highlightColor: ValkyrieColor
        get() = ValkyrieColor.WIDGET_DECLARATION
    
    override val highlightElement: PsiElement?
        get() = nameIdentifier

    override fun toString(): String {
        return "ValkyrieWidgetDeclaration(${name ?: "<anonymous>"})"
    }
}