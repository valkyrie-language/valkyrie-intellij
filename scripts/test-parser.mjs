...
const directories = [
  ...
  'src/test/resources/testData/parser/sfc_grammar'
];
...
function runTest(testClass, timeoutMs) {
  return new Promise(async (resolve, reject) => {
    ...
    const command = `${gradleCmd} test --tests "${testClass}" -x prepareTestSandbox`;
...
(async () => {
  try {
    // 运行 parser 测试
    const parserResult = await runTest('valkyrie.parser.SfcParserTest', TIMEOUT);
    ...
    if (error.message.includes('Timeout')) {
      ...
      console.log('4. DejavuParser.parseTemplate() - 如果某个 token 处理逻辑没有正确消耗 token，会导致死循环');
      console.log('\n建议检查:');
      console.log('- DejavuParser 中的 while 循环是否所有分支都能正确消耗 token');
    }
